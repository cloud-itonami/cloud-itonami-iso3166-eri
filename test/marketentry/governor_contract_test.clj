  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Asmara Textiles PLC"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Asmara Textiles PLC" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest mining-procurement-noncompliant-is-held-and-unoverridable
  (testing "declared :mining-sector? true but suppliers-solicited/sole-supplier-market? fails compliance -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:mining-procurement-noncompliant} (-> (store/ledger db) last :basis)))
...
(deftest inland-revenue-record-unverified-is-held-and-unoverridable
  (testing "unverified Inland Revenue Department tax record when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:inland-revenue-record-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
