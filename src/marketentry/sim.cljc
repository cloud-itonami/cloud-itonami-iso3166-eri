...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (ERI, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up mining-procurement-noncompliant) ==")
    ...
    (println "== filing/submit eng-4 (mining-procurement-noncompliant -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up inland-revenue-record-unverified) ==")
    ...
    (println "== filing/submit eng-5 (inland-revenue-record-unverified -> HARD hold) ==")
