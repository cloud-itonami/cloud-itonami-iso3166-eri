(ns marketentry.store
  "SSoT for the Eritrea (ERI) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:mining-sector?` / `:suppliers-solicited` / `:sole-supplier-market?`
  are ground truth for the `mining-procurement-noncompliant` flagship
  check ('the Mining Law of Eritrea': Minerals Proclamation No.
  68/1995, Mineral Proclamation No. 165/2011, Legal Notice No. 19/1995,
  Mining Income Tax Proclamation No. 69/1995); `:requires-inland-revenue-record?`
  / `:inland-revenue-record-verified?` are ground truth for the
  conditional Inland Revenue Department tax-record check. All operator
  names below are FICTIONAL demo entities -- none of them are the real
  Eritrean mining companies (e.g. Bisha Mining Share Company, Zara
  Mining Share Company) this repo's research cited."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Asmara Textiles PLC" :portal "no e-procurement portal found (see marketentry.facts)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mining-sector? false :suppliers-solicited 0 :sole-supplier-market? false
             :requires-inland-revenue-record? true :inland-revenue-record-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ERI" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "no e-procurement portal found (see marketentry.facts)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mining-sector? false :suppliers-solicited 0 :sole-supplier-market? false
             :requires-inland-revenue-record? true :inland-revenue-record-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Keren Trading Company" :portal "no e-procurement portal found (see marketentry.facts)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "ERI" :status :intake}
    "eng-4" {:id "eng-4" :operator "Adobha Mineral Exploration Ltd" :portal "no e-procurement portal found (see marketentry.facts)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mining-sector? true :suppliers-solicited 1 :sole-supplier-market? false
             :requires-inland-revenue-record? true :inland-revenue-record-verified? true
             :drafted? false :submitted? false
             :jurisdiction "ERI" :status :intake}
    "eng-5" {:id "eng-5" :operator "Massawa Freight Services" :portal "no e-procurement portal found (see marketentry.facts)"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :mining-sector? false :suppliers-solicited 0 :sole-supplier-market? false
             :requires-inland-revenue-record? true :inland-revenue-record-verified? false
             :drafted? false :submitted? false
             :jurisdiction "ERI" :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               mining-sector? suppliers-solicited sole-supplier-market?
                               requires-inland-revenue-record? inland-revenue-record-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? mining-sector?)                (assoc :engagement/mining-sector? mining-sector?)
    (some? suppliers-solicited)           (assoc :engagement/suppliers-solicited suppliers-solicited)
    (some? sole-supplier-market?)         (assoc :engagement/sole-supplier-market? sole-supplier-market?)
    (some? requires-inland-revenue-record?) (assoc :engagement/requires-inland-revenue-record? requires-inland-revenue-record?)
    (some? inland-revenue-record-verified?) (assoc :engagement/inland-revenue-record-verified? inland-revenue-record-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/mining-sector? :engagement/suppliers-solicited :engagement/sole-supplier-market?
   :engagement/requires-inland-revenue-record? :engagement/inland-revenue-record-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :mining-sector? (boolean (:engagement/mining-sector? m))
     :suppliers-solicited (:engagement/suppliers-solicited m)
     :sole-supplier-market? (boolean (:engagement/sole-supplier-market? m))
     :requires-inland-revenue-record? (boolean (:engagement/requires-inland-revenue-record? m))
     :inland-revenue-record-verified? (boolean (:engagement/inland-revenue-record-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-eri"})
